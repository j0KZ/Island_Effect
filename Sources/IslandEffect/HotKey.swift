...
struct HotKeySpec: Equatable {
    ...
    static let defaultClipboard = HotKeySpec(keyCode: kVK_ANSI_V, modifiers: optionKey | cmdKey)
...
final class GlobalHotKey {
    ...
    init(spec: HotKeySpec, action: @escaping () -> Void) {
        ...
        let status = RegisterEventHotKey(UInt32(spec.keyCode),
                                         ...
                                         &ref)
        if status != noErr {
            IslandDebug.log("no se pudo registrar el atajo \(spec.display) (status \(status))")
